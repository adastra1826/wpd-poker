...
(function () {
  ...
  const STYLES = `
    ...
    .poker-btn[data-toggled="true"] {
      ...
      box-shadow: 0 0 12px rgba(23, 162, 184, 0.8) !important;
  ...
  function syncSwappableButton(customBtn) {
    ...
    console.log(`Syncing swappable button ${label}:`, {
      isActiveInHand: state.isActiveInHand,
      isMyTurn: state.isMyTurn,
      inGame: state.gameState?.in_game,
      originalButtonActive: isOriginalButtonActive(originalId),
      preActionState: state.preActions[swapTo]
    });

    // Check for Show Hand override (for the Fold slot)
    if (hasShowHandOverride && isOriginalButtonActive("poker-SHOWHAND")) {
      console.log(`${label} -> Show Hand mode`);
    ...
    if (isOriginalButtonActive(originalId)) {
      console.log(`${label} -> Standard mode (our turn)`);
    ...
    // Otherwise disable
    console.log(`${label} -> Disabled (not active in hand or game not in progress)`);
    customBtn.textContent = label;
    customBtn.setAttribute("data-mode", "standard");
    customBtn.removeAttribute("data-toggled");
    disableButton(customBtn);
